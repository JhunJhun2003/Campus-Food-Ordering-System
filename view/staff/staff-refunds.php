...
// Check if user has view orders permission (which covers refund management)
if (!$permissions['viewOrders']) {
    $_SESSION['error'] = "You do not have permission to access staff refunds.";
...
$pageTitle = 'Staff - Manage Refunds';
$activePage = 'refunds';
...
<main class="flex-1 p-8 overflow-y-auto">
    ...
    <div class="flex justify-between items-start mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-950">Refund Requests</h1>
            <p class="text-slate-500 text-sm mt-1">Review, approve or reject customer refund requests</p>
        </div>
    </div>
    ...
    <!-- Filters and Search -->
    <div class="bg-white border border-slate-100 rounded-xl shadow-sm flex flex-col overflow-hidden mb-6">
        <div class="p-5 flex items-center justify-between border-b border-slate-50">
            <div class="relative w-full max-w-xl">
                <span class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                    <i class="fa-solid fa-magnifying-glass text-slate-400"></i>
                </span>
                <input type="text" id="searchInput" placeholder="Search refunds by Customer name or Order ID..." class="w-full pl-11 pr-4 py-2.5 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 text-sm placeholder-slate-400">
            </div>
            <div class="flex items-center space-x-3">
                <select id="statusFilter" class="px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 bg-white">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse" id="refundsTable">
                <thead>
                    <tr class="bg-slate-50 text-slate-400 font-semibold text-xs uppercase border-b border-slate-100">
                ...
                <tbody class="divide-y divide-slate-50 text-sm text-slate-600">
                    <?php if (empty($refunds)): ?>
                        <tr>
                            <td colspan="8" class="py-8 text-center text-slate-400">No refund requests found.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($refunds as $refund): ?>
                            <tr class="hover:bg-slate-50/50 transition-colors refund-row" 
                                ...
                                <td class="py-4 px-6 text-center font-medium text-slate-900">#<?php echo $refund['id']; ?></td>
